...
import TreatmentWillPanel_2 from "../panels/treatment-will-panels/TreatmentWillPanel_2";

@Component({
    template: require("./TreatmentWillTab_2.html")

})
export default class TreatmentWillTab_2 implements TabbedPanel, Render {
    private static TAB_ID = "TreatmentWillTab_TS_2";
    private static TAB_TITLE = "Behandlingstestamente_2";
    ...
    @WidgetElement private treatmentWillPanel: TreatmentWillPanel_2;
    ...
    public render(): void | Promise<never> {
        this.treatmentWillPanel = this.container.resolve(TreatmentWillPanel_2);
        this.treatmentWillPanel.render();
        this.element.appendChild(this.treatmentWillPanel.element);
    }

    public getId(): string {
        return TreatmentWillTab_2.TAB_ID;
    }

    public getTitle(): string {
        return TreatmentWillTab_2.TAB_TITLE;
    }
    ...
    public async applicationContextIdChanged(applicationContextId: string): Promise<void> {
        if (this.isApplicable(false, this.moduleContext.getUserContext())) {
            ...
            if (isTreatmentWillStarted && isFSKAdmin && isPatientContext) {
                this.moduleContext.showTab(TreatmentWillTab_2.TAB_ID);
            } else {
                this.moduleContext.hideTab(TreatmentWillTab_2.TAB_ID);
            }
        }
    }
