...
public class AccessControlImpl implements AccessControl {
    private static final Logger log = LogManager.getLogger(AccessControlImpl.class);
    ...
    public void checkOdrReadAccess() {
        ...
        if (!isAuthorized) {
            log.info("Denying access to read organdonorregistration for user " + userInfo.cpr + " in role "
    ...
    public void checkLtrBtrReadAccess() {
        ...
        if (!isAuthorized) {
            log.info("Denying access to read livingwill / treatmentwill for user " + userInfo.cpr + " in role "
                    + userInfo.requestedRole);
            throw new WebApplicationException(Response.Status.FORBIDDEN);
        }
    }
    ...
    public void checkWriteAccess() {
        ...
        if (!isAuthorized) {
            log.info("Denying access to write for user " + userInfo.cpr + " in role "
                    + userInfo.requestedRole);
            throw new WebApplicationException(Response.Status.FORBIDDEN);
        }
    }
